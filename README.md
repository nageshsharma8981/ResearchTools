...
| Journal Rankings & Comparison | `journal-rankings.html` | FT50, ABS/AJG 2024, ABDC 2025, Scopus-based quartiles (SCImago 2025), and impact metrics for 3,188 business & management journals — search, field + rating filters, compare up to 5 side by side with best-in-row highlighting, live OpenAlex citedness, CSV/PDF export. Keyless. |
| Citation Integrity Checker | `citation-integrity.html` | Upload a paper — every in-text citation checked against the reference list and vice versa: missing references, uncited entries, author/year mismatches, multi-reference groups, APA 7 style warnings (ambiguity, and/&), PDF + Markdown report. Fully in-browser, keyless. |
...
| StatPLS | `statpls.html` | Multi-method quant workbench: full PLS-SEM with AI interpretation (workflow familiar to SmartPLS users; independent implementation from the published literature), mediation (Model 4, bootstrapped indirect) & moderation (Model 1, simple slopes), and multiple regression —, fully in-browser: TAM template + auto-build from column prefixes, drag-and-drop item assignment, α/ρA/CR/AVE, outer weights, cross-loadings, Fornell–Larcker, HTMT, 5,000-resample bootstrap (custom 100–10,000), total effects, f²/VIF/R², draggable path diagram, APA write-up. Keyless; data never leaves the device. |
...
- Every AI output carries a **Verify citations** button — DOIs are checked against the Crossref registry (nonexistent DOIs flagged as likely fabricated) and reference lines are bibliographic-matched — plus a standing verify-before-use disclaimer.
