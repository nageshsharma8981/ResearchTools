...
| AI PLS | `pls-sem.html` | Multi-method quant workbench: SmartPLS-style PLS-SEM with AI interpretation, PROCESS-style mediation (Model 4, bootstrapped indirect) & moderation (Model 1, simple slopes), and multiple regression —, fully in-browser: TAM template + auto-build from column prefixes, drag-and-drop item assignment, α/ρA/CR/AVE, outer weights, cross-loadings, Fornell–Larcker, HTMT, 5,000-resample bootstrap (custom 100–10,000), total effects, f²/VIF/R², draggable path diagram, APA write-up. Keyless; data never leaves the device. |
